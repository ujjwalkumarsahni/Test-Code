...
const CreateOrderPage = () => {
  ...
  // Get unique identifier for book items
  const getBookItemId = (item) => {
    if (item.isComboPack) {
      return `COMBO_${item.bookType}_${item.grade}`;
    } else if (item.isIndividualBook) {
      return `IND_${item.bookType}_${item.grade}_${item.bookName}`;
    } else {
      return `STANDARD_${item.bookType}_${item.grade}`;
    }
  };

  // Get unique identifier for kit items
  const getKitItemId = (item) => {
    if (item.kitType === "Individual Kit") {
      return `KIT_IND_${item.kitName}`;
    } else {
      return `KIT_${item.kitType}`;
    }
  };
  ...
  const updateItem = (index, field, value, type = "order") => {
    ...
    // Recalculate discount amount if needed
    if (type === "order" || field === "unitPrice" || field === "quantity") {
  ...
  const handleSubmit = async (e) => {
    ...
    const orderData = {
      ...formData,
      discount: formData.discountAmount // Send discount amount, not percentage
    };
